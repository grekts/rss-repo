...
/**
 * Класс преобразование одних строчных данных в ругие.
 * Клас предоставляет следующий функционал:
 * - определение ссылки, к которой идет запрос, с учетом ссылок-синонимов, указанных в пользовательском конфигурационном файле
 * - получение имени запрашиваемого контроллера
 * - получение имени запрашиваемого действия
 * - получение имени запрашиваемого виджета
 * - формирование ссылки, ведущей к виду страницы
 * - разделение ссылки на домен и путь
 * - кодирование пути с русскими символами
 * - получение работающей ссылки с учетом необходимости www и протоколом передачи данных (можно использовать для проверки рабоыт ссылки)
 * - разделение текста на обзацы, опираясь на указанные теги
 * - удаление из текста указанных или всех html тегов
 * - добавления к коду внешних ссылок свойств rel="nofollow" и target="_blanck"
 * 
...
{
	/**
	 * Определяет имеются ли в конфигурационном файле ссылки-синонимы и если есть, возвращает нужную ссылку.
	...
	/**
	 * Метод формирования имени контроллера, исходя из ссылки
	 * 
	...
	/**
	 * Метод формирования имени действия в контроллере, исходя из ссылки
	 * 
	 * @param string $url Часть ссылки, где указан контроллер и имя действия (получается после проверки наличия ссылки-синонима)
	...
	/**
	 * Метод формирования имени подключаемого виджета
	 * 
	 * @param string $url Часть ссылки, где указан контроллер и имя действия (получается после проверки наличия ссылки-синонима)
	...
	/**
	 * Метод формирования ссылки на файл вида
	 * 
	 * @param string $url Часть ссылки, где указан контроллер и имя действия (получается после проверки наличия ссылки-синонима)
	...
	/**
	 * Разделение текста по указанному html тегу
	 * 
	 * @param string $data текст, подлежащий обработке
	 * @param string $inputSepatator Html тег, по которому будет делиться текст
	 * @param string $outputSepatator Разделитель, устанавливаемый в месте деления текста. Определяется в конфигурационном файле. Если не указан в пользовательском конфигурационном файле, берется из конфигурационного файла приложения (||)
	...
	/**
	 * Удаление html тегов из указанной строки
	 * 
	 * @param string $data текст, подлежащий обработке
	 * @param array $tags Массив тегов, подлежащих удалению.  Если массив не указан, удаляются все теги
	...
	/**
	 * К внешним ссылкам в тексте добавляет свойства rel=”nofollow” и target=”_blanck” и css класс
	 * 
